...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleCheckboxChange = () => {
    ...
    const previewApprovedProducts = JSON.parse(localStorage.getItem('approved-products') || '{}');
    localStorage.setItem('approved-products', JSON.stringify({ ...previewApprovedProducts, [id]: newStatus }));
  };
  ...
  return (
    ...
    >
      <Image src={image} alt={name} className="product-image" height={'150px'} aspectRatio={1.2} />
      <Box p="2">
        <Text fontWeight="bold" fontSize="sm" className="product-name" lineClamp="1">
          {name}
        </Text>
        <Text mt="2" className="product-description" fontSize={'xs'} lineClamp="2">
          {description}
        </Text>
